...
/** Статус явки клиента на бронирование. */
type AttendanceStatus = "not_arrived" | "late" | "arrived";

/** Один отзыв о клиенте (бизнес оценивает клиента). */
type CustomerReview = {
  id: string;
  text: string;
  rating: number; // 1-5
...
/**
 * Поля, которых пока нет в BusinessBookingRequest с бэкенда:
 * рейтинг клиента, количество его бронирований, его отзывы и статус явки
 * на конкретное бронирование.
 * TODO: перенести в тип BusinessBookingRequest, когда бэкенд начнёт их отдавать.
 */
type BookingCustomerExtras = {
...
type BookingWithExtras = BusinessBookingRequest & BookingCustomerExtras;

/**
 * Временный дефолт рейтинга клиента, пока бэкенд не отдаёт customerRating.
 * В макете рейтинг показывается у каждой брони, поэтому подставляем это
 * значение вместо того, чтобы скрывать блок целиком.
 * TODO: убрать, как только customerRating начнёт приходить с бэкенда.
 */
const DEFAULT_CUSTOMER_RATING = 5;
...
/** Звезда рейтинга. Используется только в модалке "Написать отзыв" (выбор оценки). */
function StarIcon({
  filled = true,
  size = 14,
}: {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden>
      <path
        ...
        stroke={filled ? "#ffc107" : "var(--border-default)"}
        strokeWidth="1.5"
      />
    </svg>
  );
...
/**
 * Декоративная левая "скобка" рейтинга — тонкая дуга слева от числа,
 * как в макете (badge вида "❴4.0❵ рейтинг").
 */
function RatingLeftBarIcon() {
  return (
    <svg width="6" height="12" viewBox="0 0 6 12" fill="none" aria-hidden>
      <path
        d="M5.5 1C5.5 1 1 2.8 1 6C1 9.2 5.5 11 5.5 11"
        stroke="currentColor"
        strokeWidth="1.3"
        strokeLinecap="round"
        strokeLinejoin="round"
...
/** Декоративная правая "скобка" рейтинга — зеркальная дуга справа от числа. */
function RatingRightBarIcon() {
  return (
    <svg width="6" height="12" viewBox="0 0 6 12" fill="none" aria-hidden>
      <path
        d="M0.5 1C0.5 1 5 2.8 5 6C5 9.2 0.5 11 0.5 11"
        stroke="currentColor"
        strokeWidth="1.3"
        strokeLinecap="round"
        strokeLinejoin="round"
...
/** Иконка "перейти" в шапке модалки профиля клиента. */
function ExternalLinkIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
      <path
        d="M14 5h5v5M19 5L10 14M9 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-3"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
        strokeLinejoin="round"
...
/**
 * Бейдж рейтинга: ❴ число ❵ рейтинг — как в макете.
 * Общий кусочек, чтобы не дублировать разметку в строке бронирования и в модалках.
 */
function RatingBadge({
  ...
  const wrapperClass =
    size === "sm" ? desktop.ratingBadgeSm : desktop.ratingBadge;

    const { t } = useTranslation();
    
  return (
    <span className={wrapperClass}>
      <span className={desktop.ratingBar}>
        <Image src={assets.profile.leftBarg} alt="" width={9} height={18} />
      </span>
      <span className={desktop.ratingValue}>{value.toFixed(1)}</span>
      <span className={desktop.ratingBar}>
        <Image src={assets.profile.rightBarg} alt="" width={9} height={18}/>
      </span>
      <span className={desktop.ratingCaption}>{t("profile.ratingLabel")}</span>
    </span>
  );
}
...
function WorkspaceTabs({
  ...
  const tabClass = (isActive: boolean) =>
    `flex-1 rounded-[12px] px-[14px] py-[12px] text-[14px] font-semibold transition lg:flex-none lg:rounded-none lg:border-b-2 lg:bg-transparent lg:px-0 lg:pb-[14px] lg:pt-0 lg:text-[16px] ${
      isActive
        ? "bg-[#0a6af7] text-white lg:border-[#0a6af7] lg:text-[#0a6af7]"
        : "bg-[var(--bg-surface)] text-[var(--text-primary)] lg:border-transparent lg:text-[var(--text-secondary)]"
    }`;

  return (
    <div
      className="mb-[16px] flex gap-[8px] rounded-[16px] bg-[var(--bg-surface-muted)] p-[6px] lg:mb-[24px] lg:gap-[32px] lg:rounded-none lg:border-b lg:border-[var(--border-default)] lg:bg-transparent lg:p-0"
...
function ItemPhoto({ photo, alt }: { photo: string | null; alt: string }) {
  return (
...
/**
 * Ряд чипов статуса явки. Показывается вместо "Принято" у подтверждённых
 * бронирований. Тексты зашиты литералами (не через t()), т.к. это временные
 * UI-элементы — так же, как "Назад" / "Состав заказа" / "Цена" в этом файле.
 */
function AttendanceStatusChips({
  value,
  onChange,
  bookingId,
}: {
  ...
  const options: {
  ...
  }[] = [
    {
      key: "not_arrived",
      label: "Не прибыл",
      className: desktop.attendanceNotArrived,
    },
    { key: "late", label: "Опаздывает", className: desktop.attendanceLate },
    { key: "arrived", label: "Прибыл", className: desktop.attendanceArrived },
  ];

  return (
    <div
      className={desktop.attendanceChips}
      data-testid={`business-booking-attendance-${bookingId}`}
    >
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            data-testid={`business-booking-attendance-${opt.key}-${bookingId}`}
            data-active={active ? "true" : "false"}
            className={`${desktop.attendanceChip} ${opt.className} ${
              active ? desktop.attendanceChipActive : ""
            }`}
          >
            {opt.label}
          </button>
...
function FieldError({
  ...
  return (
    <span
      className="text-[13px] text-[#e02424]"
      data-testid={testId}
      role="alert"
    >
      {message}
...
function AddItemScreen({
  ...
  const formTestId = isService
    ? "business-add-service-form"
    : "business-add-product-form";
    ...
    <div data-testid={formTestId}>
      ...
      <div className="flex flex-col gap-[18px]">
        ...
        <PriceField
          label={
            isService
              ? t("businessForms.servicePrice")
              : t("businessForms.price")
          }
          value={form.price}
        ...
        <label className="flex flex-col gap-[8px]">
          ...
          <div className="flex items-center justify-between gap-[12px]">
            <FieldError
              ...
              message={t("businessForms.descriptionLimitReached", {
                max: MAX_DESC,
              })}
        ...
        {isService ? (
          <>
            <div
              ...
              data-testid="business-service-time-slots"
            >
              <span className="text-[14px] font-semibold">
...
function DeleteItemModal({
  ...
  return (
    <div
      ...
      data-testid={
        isProduct
          ? "business-delete-product-modal"
          : "business-delete-item-modal"
      }
      onClick={onCancel}
    >
      <div
...
function BookingChip({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
...
/* ---------- booking card (desktop, >=1024px, matches Figma mockup) ---------- */

// Состав заказа пока не приходит в BusinessBookingRequest с бэкенда.
// Компонент готов принять его через необязательное поле `items` — как только
// бэкенд начнёт его отдавать, просто добавь `items` в тип BusinessBookingRequest,
// и блок "Состав заказа" отрисуется сам (см. bookingItems ниже).
type BookingLineItem = { name: string; qty?: number };
...
function DesktopBookingRow({
  ...
  onAccept: () => void;
  onCancel: () => void;
  onViewCustomer: (booking: BookingWithExtras) => void;
  onAttendanceChange: (
  ...
  const extras = booking as BookingWithExtras;
  // TODO: убрать fallback на DEFAULT_CUSTOMER_RATING, когда бэкенд начнёт
  // отдавать customerRating в BusinessBookingRequest.
  const customerRating = extras.customerRating ?? DEFAULT_CUSTOMER_RATING;

  const bookingItems: BookingLineItem[] =
    (booking as BusinessBookingRequest & { items?: BookingLineItem[] }).items ??
    [];
  const hasItems = bookingItems.length > 0;

  return (
    <div
      className={desktop.bookingCard}
      data-testid={`business-booking-row-${booking.id}`}
    >
      <div className={desktop.bookingRow}>
        <span className={desktop.bookingTime}>{booking.time}</span>

        <div className={desktop.bookingCustomer}>
          <p
            className={desktop.bookingCustomerName}
            data-testid={`business-booking-row-customer-${booking.id}`}
          >
            {booking.customerName}
          </p>

          <div className={desktop.bookingCustomerMeta}>
            <RatingBadge value={customerRating} size="sm" />
            <button
              type="button"
              className={desktop.bookingViewLink}
              data-testid={`business-booking-row-view-${booking.id}`}
              onClick={() => onViewCustomer(extras)}
            >
              Смотреть
        ...
        <p className={desktop.bookingService}>{booking.serviceName}</p>

        <div className={desktop.bookingPriceBlock}>
          <span className={desktop.bookingPriceLabel}>Цена</span>
          <span
            className={desktop.bookingPriceValue}
            data-testid={`business-booking-row-price-${booking.id}`}
          >
            {formatPrice(booking.price)} {t("businessForms.currencySum")}
          </span>
        </div>

        <div className={desktop.bookingActions}>
          {booking.status === "pending" && (
            <>
              <button
                type="button"
                onClick={onAccept}
                data-testid={`business-booking-row-accept-${booking.id}`}
                className={desktop.bookingAcceptBtn}
              >
                {t("business.acceptBooking")}
              </button>
              <button
                type="button"
                onClick={onCancel}
                data-testid={`business-booking-row-cancel-${booking.id}`}
                className={desktop.bookingCancelBtn}
              >
                {t("business.cancelBooking")}
              </button>
            </>
          ...
          {isConfirmed && (
            <AttendanceStatusChips
              ...
              value={extras.attendanceStatus ?? null}
              onChange={(status) => onAttendanceChange(extras, status)}
            />
          )}
      ...
      {hasItems && (
        <>
          <button
            ...
            className={desktop.bookingExpandToggle}
            aria-expanded={expanded}
            data-testid={`business-booking-row-expand-${booking.id}`}
          >
            <span className={desktop.bookingExpandLabel}>
              <span className={desktop.bookingExpandIcon}>
                <BagIcon />
              </span>
              Состав заказа
            </span>
            <span className={desktop.bookingExpandCount}>
              {bookingItems.length} товара
            </span>
            <span
              className={`${desktop.bookingExpandChevron} ${
                expanded ? desktop.bookingExpandChevronOpen : ""
              }`}
            >
              <ChevronDownIcon />
            </span>
          ...
          {expanded && (
            <div className={desktop.bookingExpandContent}>
              {bookingItems.map((item, index) => (
                <div key={index} className={desktop.bookingExpandItem}>
                  <span>{item.name}</span>
...
/* ---------- customer profile modal (Frame 641) ---------- */
...
function CustomerProfileModal({
  ...
  const { t } = useTranslation();
  const reviews = booking.customerReviews ?? [];
  const customerRating = booking.customerRating ?? DEFAULT_CUSTOMER_RATING;
  ...
  const attendanceClass = (status: AttendanceStatus) => {
    if (status === "arrived") return desktop.attendanceArrived;
    if (status === "late") return desktop.attendanceLate;
    return desktop.attendanceNotArrived;
  };

  return (
    <div
      className="fixed inset-0 z-[70] flex items-center justify-center bg-[var(--backdrop)] p-[20px]"
      data-testid="business-customer-profile-modal"
      onClick={onClose}
    >
      <div
        className="w-full max-w-[420px] rounded-[20px] bg-[var(--bg-surface)] p-[20px] shadow-[var(--shadow-modal)]"
      ...
      >
        <div className="flex items-start justify-between gap-[12px]">
          <div className="flex items-center gap-[12px]">
            <div className="relative h-[44px] w-[44px] shrink-0 overflow-hidden rounded-full bg-[var(--bg-surface-muted)]">
              <Image
                src={assets.profile.avatar}
                alt=""
                fill
            ...
            <h3 className="text-[17px] font-bold">{booking.customerName}</h3>
          </div>
          <div className="flex items-center gap-[8px]">
            <button
              type="button"
              onClick={onWriteReview}
              data-testid="business-customer-write-review-btn"
              className="rounded-[10px] bg-[#0a6af7] px-[14px] py-[8px] text-[13px] font-semibold text-white transition hover:bg-[#0858ce]"
            >
              Написать отзыв
            </button>
            <button
              ...
              onClick={onClose}
              aria-label={t("businessForms.closeAria")}
              data-testid="business-customer-profile-close"
              className="flex h-[32px] w-[32px] shrink-0 items-center justify-center rounded-full text-[var(--text-primary)] transition hover:bg-[var(--bg-surface-muted)]"
            >
              <ExternalLinkIcon />
            </button>
        ...
        <div className="mt-[10px] flex items-center gap-[12px] text-[13px] text-[var(--text-muted)]">
          <RatingBadge value={customerRating} />
          {typeof booking.customerBookingsCount === "number" && (
            <span>{booking.customerBookingsCount} бронирований</span>
          )}
        </div>

        <h4 className="mt-[18px] text-[15px] font-bold">Отзывы</h4>

        <div className="mt-[10px] flex max-h-[280px] flex-col gap-[10px] overflow-y-auto">
          {reviews.length === 0 && (
          ...
          {reviews.map((review) => (
            <div
              key={review.id}
              className="rounded-[14px] bg-[var(--bg-surface-muted)] p-[12px]"
              data-testid={`business-customer-review-${review.id}`}
            >
              ...
              <div className="mt-[8px] flex items-center justify-between">
                ...
                <span
                  className={`rounded-full px-[10px] py-[3px] text-[11px] font-semibold ${attendanceClass(
                    review.attendanceStatus,
                  )}`}
                >
                  {attendanceLabel(review.attendanceStatus)}
                </span>
              </div>
            </div>
          ))}
...
function WriteReviewModal({
  ...
  return (
    <div
      ...
      data-testid="business-write-review-modal"
      onClick={onClose}
    >
      <div
        className="w-full max-w-[420px] rounded-[20px] bg-[var(--bg-surface)] p-[20px] shadow-[var(--shadow-modal)]"
      ...
      >
        <div className="flex items-start justify-between gap-[12px]">
          <h3 className="text-[17px] font-bold">
            Написать отзыв клиенту {customerName}
          </h3>
          <button
            type="button"
            onClick={onClose}
            aria-label={t("businessForms.closeAria")}
            data-testid="business-write-review-close"
            className="flex h-[32px] w-[32px] shrink-0 items-center justify-center rounded-full text-[var(--text-primary)] transition hover:bg-[var(--bg-surface-muted)]"
          >
            <CloseModalIcon />
          </button>
        ...
        <div className="mt-[18px] flex flex-col gap-[10px]">
          <span className="text-[14px] font-semibold">Ваша оценка</span>
          <div
            className="flex gap-[8px]"
            data-testid="business-write-review-stars"
          >
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                key={star}
                type="button"
                aria-label={`${star}`}
                data-testid={`business-write-review-star-${star}`}
                onMouseEnter={() => setHoverRating(star)}
                onMouseLeave={() => setHoverRating(0)}
                onClick={() => setRating(star)}
              >
                <StarIcon filled={star <= (hoverRating || rating)} size={26} />
              </button>
            ))}
          </div>
          <span className="text-[12px] text-[var(--text-muted)]">
            Нажми на звезду чтобы оставить оценку
          </span>
        </div>
        ...
        <label className="mt-[16px] flex flex-col gap-[8px]">
          <textarea
            ...
            value={text}
            data-testid="business-write-review-textarea"
        ...
        <button
          ...
          data-testid="business-write-review-submit"
          onClick={async () => {
            setSubmitting(true);
            try {
          ...
          className="mt-[18px] w-full rounded-[14px] bg-[#0a6af7] py-4 text-[16px] font-semibold text-white transition hover:bg-[#0858ce] disabled:opacity-50"
        >
          Отправить отзыв
        </button>
      </div>
...
export default function BusinessDashboard({
  ...
  const [profileBooking, setProfileBooking] =
    useState<BookingWithExtras | null>(null);
  const [writeReviewFor, setWriteReviewFor] =
    useState<BookingWithExtras | null>(null);
  ...
  function renderInventoryRow(item: BusinessService) {
    ...
    return (
      ...
      >
        ...
        <ServiceStatusToggle
          active={item.active}
          ariaLabel={t("businessDashboard.serviceStatusAria", {
            name: item.name,
          })}
          testId={
            isService
  ...
  function renderDesktopBookingRow(booking: BusinessBookingRequest) {
    return (
      <DesktopBookingRow
        key={booking.id}
        booking={booking}
        dateLabel={bookingDateLabel}
        onViewCustomer={(b) => setProfileBooking(b)}
        onAttendanceChange={(b, status) => {
          // TODO: заменить на updateBookingAttendance(businessId, b.id, status),
          // когда появится реальный метод в business.store.
          showToast("Статус посещения обновлён", b.customerName);
        }}
        onAccept={() => {
          void updateBookingStatus(businessId, booking.id, "accepted").catch(
            (error) => {
  ...
  return (
    <>
      <div
        className="mx-auto flex w-full max-w-[640px] flex-col pb-[24px] lg:max-w-none"
      ...
      >
        ...
        {view === "bookings" && (
          <div data-testid="business-dashboard-bookings">
            ...
            <h3 className="mb-[12px] text-[18px] font-bold lg:hidden">
              {t("businessDashboard.bookingsTitle")}
            </h3>

            {/* --- <1024px: фильтры + список как были, без изменений --- */}
            <div
              className="flex flex-wrap gap-[8px] lg:hidden"
              data-testid="business-bookings-tabs"
            >
              <button
                type="button"
            ...
            <div
              className="mt-[16px] flex flex-col gap-[12px] lg:hidden"
            ...
            {/* --- >=1024px: всё «Бронирования» — одна белая карточка на фоне #F9F9FD, как в макете --- */}
            <div
              className="hidden lg:block"
              data-testid="business-bookings-desktop"
            >
              <div className={desktop.bookingsPageBg}>
                <div className={desktop.card}>
                  <div className="mb-[24px] flex items-center justify-between">
                    ...
                    <button
                      ...
                      data-testid="business-dashboard-bookings-back-desktop"
                    >
                      {t("businessForms.back")}
                    </button>
                  </div>
                  ...
                  <div
                    className="mb-[16px] flex flex-wrap gap-[8px]"
                    data-testid="business-bookings-tabs-desktop"
                  >
                    <button
                      type="button"
                      onClick={() => setBookingTab("all")}
                      data-testid="business-bookings-tab-all-desktop"
                      className={bookingTabClass(bookingTab === "all")}
                    >
                      {t("businessDashboard.bookingsTabAll")}
                    </button>
                    <button
                      type="button"
                      onClick={() => setBookingTab("pending")}
                      data-testid="business-bookings-tab-pending-desktop"
                      className={bookingTabClass(bookingTab === "pending")}
                    >
                      {t("businessDashboard.bookingsTabPending")}
                      {pendingBookings.length > 0 && (
                    ...
                    <button
                      type="button"
                      onClick={() => setBookingTab("confirmed")}
                      data-testid="business-bookings-tab-confirmed-desktop"
                      className={bookingTabClass(bookingTab === "confirmed")}
                    >
                      {t("businessDashboard.bookingsTabConfirmed")}
                      {confirmedBookings.length > 0 && (
                  ...
                  <div
                    ...
                    data-testid="business-bookings-list-desktop"
                  >
                    {business.bookingRequests.length === 0 && (
                      <p className="py-[32px] text-center text-[15px] text-[var(--text-muted)]">
                        {t("businessDashboard.emptyBookings")}
                    ...
                    {bookingTab === "all" && (
                      <>
                        {pendingBookings.map(renderDesktopBookingRow)}
                        {confirmedBookings.length > 0 && (
                          <p className="mb-[8px] mt-[8px] text-[15px] font-bold">
                            {t("businessDashboard.bookingsTabConfirmed")}
                          </p>
                        )}
                        {confirmedBookings.map(renderDesktopBookingRow)}
                        {cancelledBookings.map(renderDesktopBookingRow)}
                      </>
                    )}
                    {bookingTab === "pending" && (
                      <>
      ...
      <DeleteBusinessModal
        ...
        onConfirm={async () => {
          ...
          showToast(
            t("business.deleteSuccessTitle"),
            t("business.deleteSuccessDesc"),
          );
          onClose();
        }}
      />
      ...
      {profileBooking && (
        <CustomerProfileModal
          ...
          onWriteReview={() => {
            setWriteReviewFor(profileBooking);
      ...
      {writeReviewFor && (
        <WriteReviewModal
          customerName={writeReviewFor.customerName}
          onClose={() => setWriteReviewFor(null)}
          onSubmit={async (rating, text) => {
            // TODO: заменить на addCustomerReview(businessId, writeReviewFor.id, { rating, text }),
            // когда бэкенд начнёт принимать отзывы бизнеса о клиенте.
            showToast("Отправить отзыв", `${rating}★ ${text.slice(0, 40)}`);
          }}
        />
      )}
    </>
