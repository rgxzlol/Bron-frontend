...
import { type SavedBusiness, useLocalizedBusinessCategory } from "@/store/business.store";
...
export default function UserBusinessPanel({
  ...
  const t = useTranslations('UserBusinessPanel');
  const getLocalizedCategory = useLocalizedBusinessCategory();
  const previewImage =
        ...
        <div className={s.body}>
          <div className={s.titleRow}>
            ...
            <div className={s.rating}>
              ...
              <span className={s.ratingMuted}>{t('zeroReviews')}</span>
            </div>
          </div>

          <p className={s.category}>{getLocalizedCategory(business.category)}</p>
          <p className={s.desc}>
          ...
          <div className={s.stats}>
            <div className={s.statBox}>
              <span className={s.statLabel}>{t('address')}</span>
              <span className={s.statValue}>
                {business.address || t('tashkent')}
              </span>
            </div>
            <div className={s.statBox}>
              <span className={s.statLabel}>{t('phone')}</span>
          ...
          {activeServices.length > 0 && (
            <div className="mt-[16px]">
              <p className="mb-[10px] text-[15px] font-semibold">{t('services')}</p>
