...
const staticCards = [
  ...
  {
    id: "mc",
    logo: assets.profile.masterCard,
    logoWidth: 42,
    logoHeight: 28,
    last4: "7878",
  },
...
// TODO: убрать fallback на DEFAULT_PROFILE_RATING, когда профиль пользователя
// начнёт отдавать реальный rating с бэкенда.
...
export default function ProfilePageContent({
  ...
  type PersonalFieldErrorsState = Partial<Record<ProfilePersonalField, string>>;

  // ...

  const [personalFieldErrors, setPersonalFieldErrors] =
    useState<PersonalFieldErrorsState>({});
  ...
  async function handleChangePassword() {
    ...
    try {
      ...
      setConfirmPassword("");
      showToast(
        t("profile.passwordChangedToast"),
        t("profile.passwordChangedToastDesc"),
      );
    } catch (error) {
      setPasswordError(
    ...
    <div className={s.content}>
      ...
      {section === "personal" && (
        ...
        >
          ...
          <label
            className={`${s.field} ${personalFieldErrors.fullName ? s.fieldError : ""}`}
          >
          ...
          <label
            className={`${s.field} ${personalFieldErrors.email ? s.fieldError : ""}`}
          >
        ...
        <div className={`${s.section} ${s.sectionGrow}`}>
          <div className={s.addCardBox}>
            <div className={s.addCardLogos}>
              <Image
                src={assets.profile.visa}
                alt="VISA"
                width={48}
                height={16}
              />
              <Image
                src={assets.profile.masterCard}
                alt="Mastercard"
                width={38}
                height={25}
              />
...
function SectionHeader({
  ...
  return (
    <div
      className={
        mobileOnly ? `${s.sectionHead} ${s.mobileOnly}` : s.sectionHead
      }
    >
      <button
        type="button"
    ...
    <>
      <span className={s.ratingBadge} data-testid="profile-rating-badge">
        <span className={s.ratingBar}>
          <Image src={assets.profile.leftBarg} alt="" width={14} height={28} />
        </span>
        <span className={s.ratingValue}>{value.toFixed(1)}</span>
        <span className={s.ratingBar}>
          <Image src={assets.profile.rightBarg} alt="" width={14} height={28} />
        </span>
