...
export const CategoryDropdown: FC<CategoryDropdownProps> = ({
  ...
  function categoryLabel(category: Category) {
    const key = HOME_CATEGORY_KEYS[category.id];
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button
        ...
        className={`w-full h-[72px] px-[18px] flex items-center justify-between rounded-[17px] bg-[#FAFAFF] border-2 transition-all duration-300 cursor-pointer text-left ${
          isOpen
          ...
          <div className="flex items-center gap-3 animate-in fade-in duration-200">
            ...
            >
              <Image
                ...
                alt={categoryLabel(selectedCategory)}
                width={18}
                height={18}
              />
            </div>
            ...
            <span className="text-[14px] text-gray-500 font-normal">
              ...
              {pluralizeServices(selectedCategory.count, language)})
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-3">
            <span className="text-[16px] font-semibold text-black opacity-70">
              {t("headerFilters.required")}
            </span>
          </div>
        )}
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-[17px] shadow-xl border border-gray-100 p-2 flex flex-col gap-1 z-50 max-h-[280px] overflow-y-auto scrollbar-thin animate-in fade-in slide-in-from-top-2 duration-200">
          <button
            ...
            className={`flex items-center gap-4 w-full px-[18px] py-[12px] hover:bg-[#FAFAFF] transition-all duration-200 rounded-[12px] ${
          ...
          >
            ...
            {selectedCategory === null && (
              <span className="ml-auto text-[#0A6AF7] font-semibold animate-in zoom-in duration-200">
                ✓
              </span>
            )}
          </button>
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Image src={cat.icon} alt={label} width={18} height={18} />
                </div>
                <div className="flex flex-col text-left">
                  <span className="text-[16px] font-semibold">{label}</span>
                  <span className="text-[12px] opacity-70">
                    {cat.count} {pluralizeServices(cat.count, language)}
                  </span>
                </div>
                {isSelected && (
                  <span className="ml-auto text-[#0A6AF7] font-semibold animate-in zoom-in duration-200">
