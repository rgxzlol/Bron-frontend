  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      title: t('bookingReminderTitle'),
      description: t('bookingReminderDesc'),
      time: "09:00",
    },
    {
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-[2px] z-50 min-w-[650px] rounded-[18px] bg-white px-5 py-4 shadow-lg border border-[#f4f4f8]">
          <div className="flex pb-[19px] justify-between border-b-3 border-[#FAFAFF]">
            <h1 className="font-semibold text-[20px] text-black">{t('notifications')}</h1>
            <button
              type="button"
          ...
          </div>
          {
            notifications.length > 0 ?
              <div className="mt-3">
                <span className="block font-semibold text-[13px] text-black opacity-60 mb-[14px]">{t('today')}</span>
                ...
                >
                  ...
                  <span className="font-semibold text-[16px] text-[#0A6AF7]">{t('deleteAllRead')}</span>
                </button>
              </div> :
              <NotificationEmpty />
          }
